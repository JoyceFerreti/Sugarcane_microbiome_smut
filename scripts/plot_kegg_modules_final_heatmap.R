...
out_dir <- file.path(base_dir, "final_plot_priority_env_first")
...
module_group_means <- read_csv(file.path(tables_dir, "module_group_means_clr.csv"), show_col_types = FALSE)
kruskal_roots <- read_csv(file.path(tables_dir, "kruskal_kegg_modules_roots.csv"), show_col_types = FALSE)
kruskal_stalk <- read_csv(file.path(tables_dir, "kruskal_kegg_modules_stalk.csv"), show_col_types = FALSE)
roots_class <- read_csv(file.path(class_dir, "roots_module_classification.csv"), show_col_types = FALSE)
stalk_class <- read_csv(file.path(class_dir, "stalk_module_classification.csv"), show_col_types = FALSE)
dunn_roots <- read_csv(file.path(posthoc_dir, "dunn_kegg_modules_roots.csv"), show_col_types = FALSE)
dunn_stalk <- read_csv(file.path(posthoc_dir, "dunn_kegg_modules_stalk.csv"), show_col_types = FALSE)
...
assign_primary_category <- function(class_df) {
  class_df %>%
    ...
    mutate(
      ...
      Category = recode(PrimaryCategoryKey, !!!category_labels)
    ) %>%
    ungroup() %>%
...
prepare_rank_table <- function(class_df, kruskal_df, compartment_pattern) {
  ...
  class_primary %>%
    select(Module, Module_Name, Category, PrimaryCategoryKey, all_of(priority_order)) %>%
    ...
    arrange(factor(Category, levels = unname(category_labels)), Kruskal_rank_value, desc(CLR_Variance))
}
...
build_heatmap_objects <- function(selected_df, dunn_df, compartment_pattern) {
  ...
  letters_long <- build_letters_table(selected_df, dunn_df) %>%
    left_join(ann_df %>% select(Module, ModuleLabel), by = "Module")
...
plot_focus_heatmap <- function(prepped, title_txt, png_file, pdf_file) {
  ...
  draw(pd, x = grid::unit(0.985, "npc"), y = grid::unit(0.985, "npc"), just = c("right", "top"))
  dev.off()
...
plot_focus_heatmap(
  roots_prep,
  "KEGG modules highlighting environmental sensitivity and genotype stability - roots",
...
plot_focus_heatmap(
  stalk_prep,
  "KEGG modules highlighting environmental sensitivity and genotype stability - stalk",
